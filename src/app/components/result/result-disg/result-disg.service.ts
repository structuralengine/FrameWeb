...
export class ResultDisgService {
  ...
  public getMaxDisg(): number {
    ...
    let arryDisg2D = Object.keys(this.disg).map((key) => {
      return this.disg[key];
    });

    // 2次元のarr型を1次元へ変形
    let arryDisg1D = arryDisg2D.reduce((pre, current) => {
      pre.push(...current);
      return pre;
    }, []);

    // 最大値を含む配列要素を取得
    let m = arryDisg1D.reduce((pre, current) =>
      Math.max(pre.dx, pre.dy, pre.dz) >
      Math.max(current.dx, current.dy, current.dz)
        ? pre
        : current
    );
