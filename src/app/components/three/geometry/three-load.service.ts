...
export class ThreeLoadService {
  ...
  private createNodeLoad(targetNodeLoad: any, nodeData: object): void {
    ...
    for (const load of targetNodeLoad) {
      pMax = Math.max(pMax, Math.abs(load.tx), Math.abs(load.ty), Math.abs(load.tz));
      mMax = Math.max(mMax, Math.abs(load.rx), Math.abs(load.ry), Math.abs(load.rz));
    }
    ...
    for (const load of targetNodeLoad) {
      ...
      for (const key of ['x', 'y', 'z']) {
        ...
        // 荷重を編集する
        this.changeConcentratedLoad(arrow);
  ...
  // 節点荷重を編集するする
  private changeConcentratedLoad(target: THREE.Group): void {
  ...
  // 寸法線を編集する
  private changeDimension(target: THREE.Group, points: THREE.Vector2[], textStr: string): void {
    ...
    // 文字を描く
    const text_geo = new THREE.TextGeometry(
      textStr, {
      font: this.font,
      size: 0.1,
      height: 0.001,
    ...
    if (pos1.y > pos0.y) {
      text.position.y += 0.5 * (text.geometry.boundingBox.max.y - text.geometry.boundingBox.min.y);
    } else {
      text.position.y -= 0.5 * (text.geometry.boundingBox.max.y - text.geometry.boundingBox.min.y);
    }
    text.name = "text";
  ...
  private createTemperatureLoad(): THREE.Group {
    ...
    // 線を描く
    const line_mat = new THREE.LineDashedMaterial({ color: line_color, dashSize: 0.1, gapSize: 0.1, linewidth: 1 });
