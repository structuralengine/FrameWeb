...
public class webframe : Singleton<webframe>
{
      ...
      if (objJson.ContainsKey("node"))
      {
        ...
        foreach (string key in node1.Keys)
        {
          try
          {
            ...
            Vector3 xyz = new Vector3(x, -y, z); // unity は左手系なので, yをマイナスし 右手系に直す
  ...
  /// <summary> 格点間の最大と最小の離れ </summary>
  private float _maxNodeLangth = -1f;
  public float maxNodeLangth
  {
    get
    {
      if (_maxNodeLangth == -1f)
      {
        CalcNodeBlockScale();
      }
      return _maxNodeLangth;
    }
  }
  private float _minNodeLangth = -1f;
  public float minNodeLangth
  {
    get
    {
      if (_minNodeLangth == -1f)
      {
        CalcNodeBlockScale();
      }
      return _minNodeLangth;
    }
  }

  public bool CalcNodeBlockScale()
  {
    ...
    min_length = (float)System.Math.Sqrt(min_length);

    _maxNodeLangth = (max_length > 0) ? max_length : -1f;
    _minNodeLangth = (min_length > 0) ? min_length : -1f;
  ...
  /// <summary> 最大と最小の値 </summary>
  private float _maxI = -1f;
  public float maxI
  {
    get
    {
  ...
  public bool CalcElementScale()
  {
    ...
    {
      if (targetElementData.ContainsKey(i) == false)
      {
        continue;
      }
    ...
    _maxI = (max_value > 0) ? max_value : -1f;
    _minI = (min_value > 0) ? min_value : -1f;
